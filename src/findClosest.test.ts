...
runTests('findClosest / findClosestIndex', {
  'basic functionality': {
    'exact matches': {
      ...
      'returns first match': [{ index: 2, args: [[0, 10, 20, 30, 20], 20] }],
    },
    'no matches': {
  ...
  filterMapFn: {
    'basic mapping callback functionality works': [
      {
        index: 1,
        args: [['foo', 'hello', 'bar'], 10, (str: string) => str.length],
      },
      {
        index: 2,
        args: [
    ...
    'returning true for non-numbers throws an error': [
      (fn: typeof findClosest) =>
        // @ts-expect-error
        expect(() => fn(['10'], 10, () => true)).toThrow(),
      (fn: typeof findClosest) =>
        expect(() => fn(['10'], 10, () => false)).not.toThrow(),
      (fn: typeof findClosest) =>
        expect(() => fn([10], 10, () => true)).not.toThrow(),
      (fn: typeof findClosest) =>
        expect(() => fn([10], 10, () => false)).not.toThrow(),
    ],
    'not providing a filterMapFn parameter causes an error for non-number arrays': [
      ...
      (fn: typeof findClosest) =>
        // @ts-expect-error
        expect(() => fn([new Date(), new Date(0)], 0)).toThrow(),
    ],
    'mapping callback parameters are correct': [
      (fn: any) => {
        const array = [6]
        fn(array, 6, (value: any, i: any, collection: any) => {
          expect(value).toBe(6)
          expect(i).toBe(0)
